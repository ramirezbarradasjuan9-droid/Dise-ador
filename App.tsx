...
export default function App() {
  ...
  // Función principal de renderizado corregida para manejar cambios de ángulo
  const handleSimulate = async (angleOverride?: Angle) => {
    ...
    const targetAngle = angleOverride || angle;
    
    if (!isStageOpen) {
        setIsStageOpen(true);
        setShowControls(false); // Mantener controles ocultos al inicio para vista limpia
    }
    
    if (angleOverride) setAngle(angleOverride);

    try {
      const url = await geminiService.generateOutfitPreview(
        MI_PERFIL, selectedTop, selectedBottom, selectedFull, selectedAccessories,
        customColor, targetAngle, pose, currentUser.referenceImg, 
        makeup,
        customClothingImg
      );
      setCurrentView(url);
  ...
  const triggerVisualFeedback = (e: React.MouseEvent) => {
    ...
    setTimeout(() => setFloatingLabels(prev => prev.filter(l => l.id !== newLabel.id)), 1000);
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#010101] text-white">
      ...
      <div className="fixed inset-0 pointer-events-none z-[9999]">
        {sparkles.map(s => (
          <div key={s.id} className="sparkle-dot" style={{ left: s.x, top: s.y, backgroundColor: s.color, '--tx': `${s.tx}px`, '--ty': `${s.ty}px` } as any} />
        ))}
      ...
      <header className="h-20 glass sticky top-0 z-[500] px-6 flex items-center justify-between border-b border-white/5">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setActiveTab('CatalogBook')} className={`px-5 py-2.5 rounded-full text-[9px] font-bold uppercase transition-all ${activeTab === 'CatalogBook' ? 'bg-amber-500 text-black shadow-lg' : 'text-neutral-500 hover:text-white'}`}>Catálogo</button>
        </div>
      </header>
      ...
      <main className="flex-grow flex flex-col lg:flex-row relative overflow-hidden">
        <aside className="w-full lg:w-[380px] bg-[#050505] border-b lg:border-r border-white/5 p-8 flex flex-col gap-10 shrink-0 overflow-y-auto no-scrollbar">
          ...
          <section className="space-y-4">
            ...
            <div className="flex items-center gap-4 bg-neutral-900 border border-white/10 rounded-xl p-3">
              <input type="color" value={customColor} onChange={(e) => setCustomColor(e.target.value)} className="w-10 h-10 rounded-lg cursor-pointer bg-transparent border-none" />
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <select value={makeup.lipstick} onChange={(e) => setMakeup({...makeup, lipstick: e.target.value})} className="w-full bg-neutral-900 border border-white/10 rounded-xl px-4 py-3 text-[10px] uppercase font-bold tracking-widest outline-none">
            ...
            <div className="space-y-2">
              ...
              <select value={pose} onChange={(e) => setPose(e.target.value as Pose)} className="w-full bg-neutral-900 border border-white/10 rounded-xl px-4 py-3 text-[10px] uppercase font-bold tracking-widest outline-none">
                {POSES.map(p => <option key={p.id} value={p.id}>{p.icon} {p.label}</option>)}
              </select>
            </div>
          </section>

          <button onClick={() => handleSimulate()} disabled={isGenerating} className="w-full btn-gold py-5 rounded-2xl text-[10px] font-bold uppercase tracking-[0.3em] shadow-2xl mt-auto disabled:opacity-50">
        ...
        <section className="flex-grow bg-[#020202] relative overflow-hidden flex flex-col">
          {activeTab === 'CatalogBook' ? (
             <div className="flex-grow flex flex-col p-8 animate-in fade-in duration-700 overflow-y-auto no-scrollbar">
               <nav className="flex gap-8 border-b border-white/5 pb-4 mb-8 overflow-x-auto no-scrollbar shrink-0">
                  {CATEGORIES.map(cat => (
                    <button key={cat} onClick={() => { setActiveCategory(cat); setCatalogPage(0); }} className={`text-[9px] font-bold uppercase tracking-widest whitespace-nowrap ${activeCategory === cat ? 'text-amber-500' : 'text-neutral-500 hover:text-white'}`}>{cat}</button>
                  ))}
               </nav>
               <div className="flex-grow flex items-center justify-center perspective-[2000px]">
                 <div className="relative w-full max-w-[1100px] aspect-[16/9] flex bg-neutral-900/50 rounded-2xl overflow-hidden border border-white/5 shadow-2xl">
                    {currentItems.map((item, idx) => (
                      <div key={item.id} onClick={(e) => selectOutfit(item, e)} className={`flex-1 relative group cursor-pointer border-r border-white/5 last:border-r-0 ${isSelected(item) ? 'selected-breathing' : ''}`}>
                    ...
                    <button onClick={() => navigateCatalog('prev')} className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/60 border border-white/10 flex items-center justify-center hover:bg-amber-500 hover:text-black transition-all">←</button>
                    <button onClick={() => navigateCatalog('next')} className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/60 border border-white/10 flex items-center justify-center hover:bg-amber-500 hover:text-black transition-all">→</button>
                 </div>
               </div>
             </div>
          ) : (
            <div className="p-8 lg:p-16 space-y-12 overflow-y-auto no-scrollbar flex-grow">
               ...
               <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-6 pb-20">
                 {filteredCatalog.slice(0, 10).map(item => (
                   <div key={item.id} onClick={(e) => selectOutfit(item, e)} className={`aspect-[3/4] rounded-3xl overflow-hidden border transition-all duration-500 cursor-pointer group relative ${isSelected(item) ? 'selected-breathing scale-105 border-amber-500' : 'border-white/5 grayscale hover:grayscale-0'}`}>
      ...
      {/* RENDER MODAL - SOLUCIÓN A OBSTRUCCIÓN Y ÁNGULOS */}
      {isStageOpen && (
        <div className="fixed inset-0 z-[2000] bg-black flex flex-col animate-in fade-in duration-700">
           <header className="h-20 glass flex items-center justify-between px-8 shrink-0">
             <div className="flex items-center gap-4">
                ...
                <span className="text-[10px] font-mono tracking-widest uppercase text-white/40">ESTUDIO_RENDER_VIVO</span>
             </div>
             <div className="flex gap-4">
                <button 
                  onClick={() => setShowControls(!showControls)}
                  className={`px-6 py-2.5 rounded-full border border-white/10 text-[9px] font-bold uppercase transition-all flex items-center gap-2 ${showControls ? 'bg-amber-500 text-black border-amber-500' : 'hover:bg-white/5 text-neutral-400'}`}
                >
                  📸 Cámara
                </button>
                <button onClick={() => setIsStageOpen(false)} className="text-[9px] font-bold uppercase text-red-500 border border-red-500/20 px-6 py-2.5 rounded-full hover:bg-red-500/10 transition-colors">Cerrar</button>
             </div>
           </header>

           <div className="flex-grow relative flex items-center justify-center overflow-hidden bg-[#020202]">
              {/* Panel de Controles Colapsable para no obstruir el rostro */}
              {showControls && (
                <div className="absolute bottom-32 right-8 w-64 bg-neutral-900/90 backdrop-blur-3xl border border-white/10 p-6 rounded-[2rem] shadow-2xl space-y-6 z-[2100] animate-pop">
                  ...
                  <button onClick={() => { setZoom(1); setRotation(0); }} className="w-full py-3 rounded-xl border border-white/10 text-[9px] font-bold uppercase text-neutral-400 hover:text-white transition-colors">Resetear Cámara</button>
                </div>
              )}

              {/* Contenedor de Imagen Central */}
              <div className="relative w-full max-w-[500px] aspect-[9/16] bg-neutral-950 rounded-[3rem] overflow-hidden border border-white/5 shadow-2xl">
                 {isGenerating ? (
                   <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/40 backdrop-blur-sm p-12 text-center space-y-6">
                      <div className="w-16 h-16 border-t-2 border-amber-500 rounded-full animate-spin" />
                      <p className="text-[9px] font-bold uppercase tracking-[0.4em] text-amber-500 animate-pulse">Renderizando nuevo ángulo...</p>
                   </div>
                 ) : null}
                 
                 {currentView ? (
                   <div className="w-full h-full" style={{ transform: `scale(${zoom}) rotate(${rotation}deg)`, transition: 'transform 0.4s cubic-bezier(0.16, 1, 0.3, 1)' }}>
                     <img src={currentView} className="w-full h-full object-cover" alt="Render Actual" />
                   </div>
                 ) : (
                    <div className="w-full h-full flex items-center justify-center text-neutral-600 text-[10px] uppercase tracking-widest">Cargando escenario...</div>
                 )}
              </div>

              {/* Botones de Ángulo (Corregidos para disparar handleSimulate) */}
              <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex gap-3 bg-neutral-900/80 backdrop-blur-2xl p-2 rounded-full border border-white/10 shadow-2xl">
                 {ANGLES.map(a => (
                   <button 
                    key={a.name} 
                    onClick={() => handleSimulate(a.name as Angle)}
                    disabled={isGenerating}
                    className={`px-6 py-3 rounded-full text-[9px] font-bold uppercase transition-all whitespace-nowrap ${angle === a.name ? 'bg-amber-500 text-black shadow-lg scale-105' : 'text-neutral-500 hover:text-white hover:bg-white/5'}`}
                   >
                     {a.name}
                   </button>
                 ))}
